  ...
  return (
    <StudentDashboardLayout>
      ...
      <div className="overflow-hidden rounded-xl border bg-white">
        ...
        {payments.map((p) => (
          <div key={p.id} className="grid grid-cols-4 items-center px-4 py-3 text-sm text-gray-700">
            <div className="font-medium">{p.id}</div>
            <div>{p.date}</div>
            <div>{p.amount}</div>
            <div className="text-right">
              <span className={`rounded-md px-2 py-1 ${p.status === 'Paid' ? 'bg-green-50 text-green-700' : 'bg-yellow-50 text-yellow-700'}`}>{p.status}</span>
            </div>
          </div>
        ))}
      </div>
